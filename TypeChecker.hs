...
--------------------------------
--- Un operador 
checkProb :: (MonadState m, MonadError m) => VarAle -> OpComp -> ExpNum -> m (VarAle -> OpComp -> ExpNum)
...
checkProbBt :: (MonadState m, MonadError m) => 
    VarAle -> OpComp -> ExpNum -> OpComp -> ExpNum 
    -> m (VarAle, OpComp, ExpNum, OpComp, ExpNum)
...
---------------------------
checkMaxP :: (MonadState m, MonadError m) =>  VarAle  -> m VarAle
checkMaxP x@(Disc _) = return x 
checkMaxP _ = throw DiscVarExpected


checkMaxFDP :: (MonadState m, MonadError m) =>  VarAle  -> m VarAle
checkMaxFDP (Cont (Unif _ _)) = throw NotDefinded
checkMaxFDP v@(Cont _) = return v
checkMaxFDP _  = throw ContVarExpected 


checkFDP ::(MonadState m, MonadError m) =>  VarAle -> NumC -> m (VarAle, NumC)
checkFDP v@(Cont _) x@(ConstD _) = return (v, x)
checkFDP v@(Cont _) x@(ConstI _) = return (v, toDouble(x))
checkFDP _ _ = throw ContVarExpected

------------------------------------
-- Checker de variables aleatorias
------------------------------------
checkVarAle ::  (MonadState m, MonadError m) => VarAle -> m VarAle
checkVarAle (Disc x) = do checkVarDisc (convert x)
                          return (Disc x')
